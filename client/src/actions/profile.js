...
export const getCurrentProfile = () => async dispatch => {
  try {
    ...
    console.error('Failed to fetch profile:', err); // More comprehensive error logging
    dispatch({
      type: PROFILE_ERROR,
      payload: {
  ...
  async dispatch => {
    try {
      ...
      dispatch({
        type: PROFILE_ERROR,
        payload: {
          msg: err.response && err.response.data && err.response.data.errors ? err.response.data.errors.map(error => error.msg).join(', ') : 'Error with request',
